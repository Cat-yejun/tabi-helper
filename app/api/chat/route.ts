...
// 여행 비서: 대화 히스토리(+선택적 이미지)를 받아 다음 답변 생성
export async function POST(req: NextRequest) {
  try {
    ...
    const sys =
      ...
      "사진을 보내면 그 안의 음식·표지판·상품·장소가 무엇인지 설명하고 여행에 도움되는 정보를 덧붙여. " +
      "답은 한국어로, 모바일 채팅에서 읽기 쉽게 간결하게. 필요하면 짧은 일본어 표현과 발음을 함께 줘. " +
      "서식은 최소화해: 표·큰제목(#)·코드블록은 쓰지 말고, 꼭 필요할 때만 - 목록이나 **굵게**를 가볍게 써. " +
      "확실하지 않은 최신 정보(운영시간·요금)는 추정임을 밝히고 현장 확인을 권해.";

    // 이미지가 포함된 메시지는 멀티모달 content 로 변환
    const apiMessages = messages.map((m) => {
      if (m.role === "user" && m.image) {
        const { mediaType, data } = parseDataUrl(m.image);
        return {
          role: "user" as const,
          content: [
            { type: "image" as const, source: { type: "base64" as const, media_type: mediaType as any, data } },
            { type: "text" as const, text: m.content || "이 사진에 대해 설명해줘." },
          ],
        };
      }
