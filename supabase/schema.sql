...
create table if not exists journal_entries (
  ...
  itinerary_id uuid references itineraries(id) on delete cascade,
  created_at timestamptz default now(),
  entry_date date,           -- 그 기록의 날짜
  entry_time text,           -- "HH:MM" (선택)
  place text,                -- 장소명(선택)
  photo_url text,            -- 사진(선택)
  caption text,              -- 본문/캡션(AI 요약 또는 직접 작성)
  sort_order int default 0
);
alter table journal_entries enable row level security;
drop policy if exists "own_all" on journal_entries;
create policy "own_all" on journal_entries for all
  using (auth.uid() = user_id) with check (auth.uid() = user_id);
-- 팀 멤버도 같은 여행의 기록을 보고 추가할 수 있게
drop policy if exists "member_all" on journal_entries;
create policy "member_all" on journal_entries for all
  using (itinerary_id is not null and public.is_trip_member(itinerary_id))
  with check (itinerary_id is null or public.is_trip_member(itinerary_id) or auth.uid() = user_id);
